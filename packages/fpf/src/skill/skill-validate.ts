...
function validatePatternString(obj: Record<string, unknown>, key: string, path: string, pattern: RegExp, label: string, errors: SchemaError[]): void {
  ...
  if (value.length === 0) {
    errors.push({ path, message: "must be non-empty" });
    return;
  }
  if (!pattern.test(value)) {
    errors.push({ path, message: `must match ${label}` });
  }
}
...
function validateSemver(obj: Record<string, unknown>, key: string, path: string, errors: SchemaError[]): void {
  ...
  if (!semverPattern.test(value)) {
    errors.push({ path, message: "must be valid semver" });
  }
}

function validateString(obj: Record<string, unknown>, key: string, path: string, errors: SchemaError[], optional = false): void {
  if (!(key in obj)) {
    if (!optional) {
      errors.push({ path, message: "missing required string" });
    }
    return;
  }
  if (typeof obj[key] !== "string") {
  ...
  if (!optional && obj[key].trim().length === 0) {
    errors.push({ path, message: "must be non-empty" });
  }
}
...
function validateStringArray(value: unknown, path: string, errors: SchemaError[], optional = false): void {
  if (value === undefined || value === null) {
    if (!optional) {
      errors.push({ path, message: "missing required array" });
    }
    return;
  }
  ...
  value.forEach((entry, index) => {
    ...
    if (!optional && entry.trim().length === 0) {
      errors.push({ path: `${path}[${index}]`, message: "must be non-empty" });
    }
  });
}
...
function validateDate(obj: Record<string, unknown>, key: string, path: string, errors: SchemaError[], optional = false): void {
  if (!(key in obj)) {
    if (!optional) {
      errors.push({ path, message: "missing required date" });
    }
    return;
  }
  if (typeof obj[key] !== "string") {
  ...
  if (!isIsoDate(obj[key])) {
    errors.push({ path, message: "expected ISO date (YYYY-MM-DD)" });
  }
}

function validateDateTime(obj: Record<string, unknown>, key: string, path: string, errors: SchemaError[], optional = false): void {
  if (!(key in obj)) {
    if (!optional) {
      errors.push({ path, message: "missing required datetime" });
    }
    return;
  }
  if (typeof obj[key] !== "string") {
  ...
  if (!isIsoDateTime(obj[key])) {
    errors.push({ path, message: "expected ISO datetime" });
  }
}
...
function checkAllowedKeys(obj: Record<string, unknown>, allowed: string[], path: string, errors: SchemaError[]): void {
  for (const key of Object.keys(obj)) {
    if (!allowed.includes(key)) {
      errors.push({ path: `${path}.${key}`, message: "unexpected key" });
    }
  }
}

function checkRequiredKeys(obj: Record<string, unknown>, required: string[], path: string, errors: SchemaError[]): void {
  for (const key of required) {
    if (!(key in obj)) {
      errors.push({ path: `${path}.${key}`, message: "missing required key" });
    }
  }
}
