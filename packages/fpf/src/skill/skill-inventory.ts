...
import { findSkillFiles, loadJsonFile, parseYaml } from "./skill-io.ts";
...
export function generateSkillInventoryMarkdown(options: { rootDir: string }): string {
  ...
  const entries = skillFiles.map((filePath) => buildEntry(filePath, options.rootDir));
  entries.sort((a, b) => a.id.localeCompare(b.id));
  return renderInventory(entries);
}
