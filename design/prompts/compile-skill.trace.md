...
HARD RULES:
1) Do NOT invent facts. If SOURCE_TEXT does not provide info, write "UNKNOWN" (or empty array/object as allowed) AND list it in compile-report.json.
...
TRACE MODE EVIDENCE RULES:
- Include provenance.field_evidence entries for:
  - $.intent.goal
  - each $.procedure[*].instruction
  - each $.eval.acceptance_criteria[*]
- Each evidence entry must be { "json_path": "...", "quote": "..." } where quote is <= 200 chars from SOURCE_TEXT or "UNKNOWN".
...
- Evidence entries present for required claim fields.
...
- If a check fails, fix it before output; if impossible, state in compile-report.json.

OUTPUT FORMAT (exact):
- First fenced block: ```json ... ``` containing ONLY the SkillSpec JSON
- Second fenced block: ```json ... ``` containing ONLY compile-report.json with:
  { "missing_required": [...], "assumptions": [...], "evidence": [{ "json_path": "...", "quote": "..." }], "evidence_warnings": [...], "evidence_coverage": 0, "validation": { "passed": true, "errors": [...] }, "notes": "..." }
