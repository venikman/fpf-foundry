...
const skillsRoot = path.join(repoRoot, "design", "skills");
const codeRoot = path.join(repoRoot, "develop", "skills", "src");
...
function parseInventory(filePath, errorsList) {
  ...
  if (missingColumns.length > 0) {
    errorsList.push(`Inventory table missing columns in ${toRepoRelative(filePath)}: ${missingColumns.join(", ")}`);
    return [];
  }
  ...
  for (let i = headerIndex + 2; i < lines.length; i++) {
    ...
    if (values.length !== numColumns) {
      errorsList.push(`${toRepoRelative(filePath)}:${i + 1} expected ${numColumns} columns, got ${values.length}`);
      continue;
    }
...
function lintInventory(rows, errorsList, filePath) {
  ...
  for (const row of rows) {
    ...
    if (!allowedStatus.has(row.status)) {
      errorsList.push(`${toRepoRelative(filePath)}:${row.lineNumber} invalid Status '${row.status}' for '${row.skillId}'`);
    }

    if (!allowedImpl.has(row.impl)) {
      errorsList.push(`${toRepoRelative(filePath)}:${row.lineNumber} invalid Impl '${row.impl}' for '${row.skillId}'`);
    }

    const policyRealization = row.policyRealization.trim();
    if (policyRealization.length === 0) {
      errorsList.push(`${toRepoRelative(filePath)}:${row.lineNumber} missing PolicyRealization for '${row.skillId}'`);
      ...
      if (policyRealization !== "passive" && !isAuditRef) {
        errorsList.push(`${toRepoRelative(filePath)}:${row.lineNumber} invalid PolicyRealization '${policyRealization}' for '${row.skillId}'`);
      }
    }
    ...
    if (row.patternRefsRaw !== "-") {
      ...
      if (refs.some((ref) => ref.length === 0)) {
        errorsList.push(`${toRepoRelative(filePath)}:${row.lineNumber} empty PatternRefs entry for '${row.skillId}'`);
        continue;
      }

      for (const ref of refs) {
        if (!patternRefRegex.test(ref)) {
          errorsList.push(`${toRepoRelative(filePath)}:${row.lineNumber} invalid PatternRef '${ref}' for '${row.skillId}'`);
        }
      }
    }
  }
