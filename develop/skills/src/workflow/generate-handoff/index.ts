...
const { values } = parseArgs({
  ...
  options: {
    ...
    "prohibited-actions": { type: "string" },
...
if (!values.context || !values["session-id"] || !values["to-agent-type"] || !values.instructions) {
  printUsage();
  process.exit(1);
}
...
if (instructions.length === 0) {
  console.error("Missing instructions. Provide at least one actionable instruction.");
  process.exit(1);
}
...
const prohibitedActions = parseSemicolonList(values["prohibited-actions"]);
if (constraints.length === 0 && prohibitedActions.length === 0) {
  console.error("Missing guard rails. Provide constraints or prohibited_actions.");
  process.exit(1);
}
...
validateAgentType(fromAgentType, "from-agent-type");
validateAgentType(toAgentType, "to-agent-type");
...
logWork({
  ...
  context,
  roleAssignment,
  action: `Generated handoff '${handoffId}' to ${toAgentType}.`,
...
function renderHandoffYaml(input: {
  ...
  const lines = [
    ...
    renderYamlKeyList("prohibited_actions", input.prohibitedActions),
...
function validateAgentType(value: string, label: string): void {
  ...
  if (!allowed.has(value)) {
    console.error(`Invalid ${label} '${value}'. Expected one of strategist, executor, proxy-auditor, human.`);
    process.exit(1);
  }
}
...
function printUsage(): void {
  console.log(
    "Usage: bun develop/skills/src/workflow/generate-handoff/index.ts --context <Context> --session-id <Id> --to-agent-type <Type> --instructions \"a; b\" [--from-agent-type <Type>] [--acceptance-criteria \"a; b\"] [--constraints \"a; b\"] [--prohibited-actions \"a; b\"] [--artifacts \"a; b\"] [--notes \"...\"] [--agent-model <Model>] [--role-assignment <Role>] [--decisions \"...\"] [--timestamp-start <iso>]",
  );
}
