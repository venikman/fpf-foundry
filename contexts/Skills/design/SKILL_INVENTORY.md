# FPF Skill Inventory (Design-Time Registry)

## Inventory schema (v1)

Columns
- Skill ID: canonical ID, also used as path segment under `contexts/Skills/src/`.
...
- Family: optional taxonomy. If kept, it MUST mean "capability area" (not a duplicate of namespace).

| Skill ID | Family | PatternRefs | Status | Impl | Outputs | Description |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `telemetry/log-work` | Telemetry | A.15.1 | experimental | code | - | Generates a U.Work record. |
| `metric/evaluate-cslc` | Metric | A.18 | planned | none | - | Evaluates a Characteristic (CSLC). |
| `design/mint-name` | Design | F.18 | experimental | code | - | Mints a new Name Card. |
| `design/record-drr` | Design | E.9 | experimental | code | - | Records a Design Rationale (DRR). |
| `reasoning/abductive-loop` | Reasoning | B.5.2 | planned | none | - | Runs the Explore-Shape-Evidence-Operate loop. |
| `audit/audit-lexicon` | Audit | E.5.1 | planned | none | - | Enforces Lexical Firewall rules. |
...
| `audit/verify-evidence` | Audit | A.10 | planned | none | - | Verifies audit trails and evidence links. |
...
| `audit/audit-naming` | Audit | E.10 | planned | none | - | Audits compliance with LEX-BUNDLE naming. |
...
| `audit/validate-scr` | Audit | F.15 | planned | none | - | Validates SCR/RSCR Harness. |
...
| `audit/benchmark-parity` | Audit | G.9 | planned | none | - | Runs a Parity Benchmark. |
